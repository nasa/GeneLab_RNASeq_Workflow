...
params:
  ...
  use_counts_table: FALSE # set to TRUE when starting from pre-made counts table CSV
  counts_table_file: "" # path to counts table CSV (for counts_table entry point)
...
if (params$use_counts_table) {
    # For counts_table entry point, load pre-made CSV directly
    cat("=== COUNTS_TABLE ENTRY POINT MODE ===\n")
    cat("Loading counts table from:", params$counts_table_file, "\n")
    
    if (!file.exists(params$counts_table_file)) {
        stop(paste("Counts table file not found at:", params$counts_table_file))
    }
    
    counts <- read.csv(params$counts_table_file, header = TRUE, row.names = 1, check.names = FALSE)
    cat("Loaded counts table with", nrow(counts), "genes and", ncol(counts), "samples\n")
    cat("Sample names in counts table:", paste(colnames(counts), collapse=", "), "\n")
    cat("Sample names in runsheet:", paste(rownames(study), collapse=", "), "\n")
    
    # Reorder counts columns to match runsheet
    counts <- counts[, rownames(study)]
    
    # Round to integers (RSEM outputs decimal counts, DESeq2 needs integers)
    counts <- round(counts)
    cat("Rounded counts to integers for DESeq2\n")
    cat("Final counts matrix:", nrow(counts), "genes x", ncol(counts), "samples\n")
} else if (params$microbes) {
    ...
    reordering <- sapply(samples, function(x) {
        grep(paste0("^", x, params$output_filename_suffix, "\\.genes.results$"), clean_filenames, value = FALSE)
    })
...
if (params$DEBUG_MODE_LIMIT_GENES) {
    if (params$use_counts_table || params$microbes) {
        counts <- counts[1:150,]
    } else {
...
if (params$use_counts_table || params$microbes) {
    # No txi.rsem length adjustment needed for counts matrix mode
} else {
...
rownames(sampleTable) <- if (params$use_counts_table || params$microbes) {
    colnames(counts)
} else {
...
# Create dataset
if (params$use_counts_table || params$microbes) {
    # Build DESeqDataSet directly from counts matrix
    # (for counts_table entry point or microbes mode)
    cat("Building DESeqDataSet from counts matrix\n")
    ...
    # Build DESeqDataSet from tximport object for default RSEM mode
    cat("Building DESeqDataSet from tximport object\n")
